    ...
    <div className="p-4 bg-white shadow rounded-lg flex flex-col">
      <Label className="text-sm text-gray-600 mb-2 w-full">
        ...
        <span className="ml-2 font-medium text-black">
          {isDelivery ? "Delivery" : "Ballarat Pickup"}
        </span>
      </Label>
      {isDelivery && (
        <Label className="text-sm text-gray-600 mb-2 w-full">
          Dropoff Address:
          <span className="ml-2 font-medium text-black">{address}</span>
        </Label>
      )}

      <Label className="text-sm text-gray-600 w-full">
        Hire date:{" "}
        <span className="ml-2 font-medium text-black">{formattedDate}</span>
      </Label>
    </div>
  );
};
